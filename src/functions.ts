...
export async function run() {
  ...
  if (!resp) {
    console.error('Failed to check and update DNSRecords.');
    return;
  }
  if (resp.dnsIP !== resp.publicIP) {
    ...
    if (publicIP === dnsIP) {
      ...
      records.push(
        await updateDNSRecord(
          ...
          process.env.HAMRODEV_ID as string,
          publicIP,
        ),
      );
      records.push(
        await updateDNSRecord(
          process.env.ZONE as string,
          process.env.WWW_HAMRODEV_ID as string,
          publicIP,
        ),
      );
    }
...
export async function notifyIPtoTelegram(dns: DNSRecordIPResponse): Promise<boolean> {
  ...
  if (!botToken || !chatId) {
    throw new Error('Faltan TELEGRAM_BOT_TOKEN o TELEGRAM_CHAT_ID en variables de entorno');
  }
